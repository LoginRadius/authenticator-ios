...
@interface LRAuthListVC ()<UITableViewDelegate,UITableViewDataSource,SWRevealViewControllerDelegate,UITextFieldDelegate>
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    LRURL *url = [self.authURLs objectAtIndex:indexPath.row];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"toLRAuthDetail"]) {
        LRAuthDetailVC *vc = (LRAuthDetailVC *)segue.destinationViewController;
        vc.authURL = sender;
    }
    
    else if([segue.identifier isEqualToString:@"ListToAddURLVC"]) {
        LRWelcomeVC *vc = (LRWelcomeVC *)segue.destinationViewController;
        vc.isBack = YES;
    }
...
- (IBAction)pressMenu:(UIButton *)sender {
    LRURL *currentURL = ((LRURL *)[_authURLs objectAtIndex:sender.tag]);
    LRURLSession *uSession = [[LRURLSession alloc] init];
...
- (void)addCategoryFromAddToCategory:(LRURL *)url {
    LRURLSession *uSession = [[LRURLSession alloc] init];
    ...
    UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"Submit" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if([catArr containsObject:[[alertController textFields][0] text]]) {
            ...
            if(url) {
                ...
                [CommonOps showToastOnVC:self message:[NSString stringWithFormat:@"Added to %@", url.category]];
            }
