...
static NSString *const kTOTPScheme = @"totp";
...
NSString *const OTPURLWillGenerateNewOTPWarningNotification
= @"URLWillGenerateNewOTPWarningNotification";
NSString *const OTPURLDidGenerateNewOTPNotification
= @"URLDidGenerateNewOTPNotification";
NSString *const OTPURLSecondsBeforeNewOTPKey
= @"URLSecondsBeforeNewOTP";

@interface OTPURL ()

// re-declare readwrite

// Initialize an URL with a dictionary of attributes from a keychain.
+ (OTPURL *)authURLWithKeychainDictionary:(NSDictionary *)dict;

// Initialize an URL object with an :// NSURL object.
...
+ (OTPURL *)authURLWithURL:(NSURL *)url
    ...
    if ([urlScheme isEqualToString:kTOTPScheme]) {
        // Convert totp:// into otpauth://
        authURL = [[TOTPAuth alloc] initWithTOTPURL:url] ;
    } else if (![urlScheme containsString:kOTPScheme]) {
        // Required (otpauth://)
    } else {
        ...
        if ([path length] > 1) {
            ...
            NSDictionary *query =
            [StringOperations dictionaryWithQueryString:[url query]];
            ...
            if (!secret) {
                ...
                if([query objectForKey:kQuerySecretKey]){
                    ...
                    secret = [StringOperations decode:secretString];
                }
                else {
                    return nil;
                }
...
+ (OTPURL *)authURLWithKeychainItemRef:(NSData *)data {
    OTPURL *authURL = nil;
...
+ (NSData *)base64Decode:(NSString *)string {
    return [StringOperations decode:string];
    
}
+ (NSString *)encodeBase64:(NSData *)data {
    return  [StringOperations encode:data];
}
- (id)initWithOTPGenerator:(OTPGenerator *)generator
                      name:(NSString *)name {
    if ((self = [super init])) {
...
// a subset of the functionality available in :// urls, so we just
...
- (id)initWithTOTPURL:(NSURL *)url {
    ...
    NSData *secret = [StringOperations decode:[url fragment]];
...
- (void)totpTimerFunc:(NSTimer *)timer {
    ...
    if (progress == 0 || progress > self.lastProgress) {
        [nc postNotificationName:OTPURLDidGenerateNewOTPNotification object:self];
        self.lastProgress = period;
        self.warningSent = NO;
        ...
        = [NSDictionary dictionaryWithObject:warning
                                      forKey:OTPURLSecondsBeforeNewOTPKey];
        
        [nc postNotificationName:OTPURLWillGenerateNewOTPWarningNotification
                          object:self
                        userInfo:userInfo];
...
- (NSURL *)url {
    ...
    NSLog(@"%@",[NSString stringWithFormat:@"%@://totp/%@?%@",
                 kOTPScheme,
                 [StringOperations URLEncodedString:self.name],
                 [StringOperations queryStringFromDcit:query]]);
