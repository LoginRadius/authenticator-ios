...
NSString *const LRStringEncodingErrorDomain = @"com.google.LRStringEncodingErrorDomain";
NSString *const LRStringEncodingBadCharacterIndexKey = @"LRStringEncodingBadCharacterIndexKey";
...
@implementation LRStringEncoding
...
+ (id)hexStringEncoding {
  LRStringEncoding *ret = [self stringEncodingWithString:
      @"0123456789ABCDEF"];
...
- (NSString *)encode:(NSData *)inData error:(NSError **)error {
  ...
  if (!value) {
    if (error) {
      *error = [NSError errorWithDomain:LRStringEncodingErrorDomain
                                   code:LRStringEncodingErrorUnableToConverToAscii
                               userInfo:nil];
...
- (NSString *)encodeString:(NSString *)inString error:(NSError **)error {
  ...
  if (!data) {
    if (error) {
      *error = [NSError errorWithDomain:LRStringEncodingErrorDomain
                                   code:LRStringEncodingErrorUnableToConverToUTF8
                               userInfo:nil];
...
- (NSData *)decode:(NSString *)inString error:(NSError **)error {
        ...
        if (error) {
          NSDictionary *userInfo =
              [NSDictionary dictionaryWithObject:[NSNumber numberWithUnsignedInteger:i]
                                          forKey:LRStringEncodingBadCharacterIndexKey];
          *error = [NSError errorWithDomain:LRStringEncodingErrorDomain
                                       code:LRStringEncodingErrorUnknownCharacter
                                   userInfo:userInfo];
        }
        return nil;
      }
          ...
          if (error) {
            NSDictionary *userInfo =
                [NSDictionary dictionaryWithObject:[NSNumber numberWithUnsignedInteger:i]
                                            forKey:LRStringEncodingBadCharacterIndexKey];
            *error = [NSError errorWithDomain:LRStringEncodingErrorDomain
                                         code:LRStringEncodingErrorExpectedPadding
                                     userInfo:userInfo];
          }
          return nil;
        }
  ...
  if (bitsLeft && buffer & ((1 << bitsLeft) - 1)) {
    if (error) {
      *error = [NSError errorWithDomain:LRStringEncodingErrorDomain
                                   code:LRStringEncodingErrorIncompleteTrailingData
                               userInfo:nil];
