...
@interface LRStringEncoding : NSObject {
 @private
  NSData *charMapData_;
  char *charMap_;
...
// Create a new, autoreleased LRStringEncoding object with a standard encoding.
...
// Create a new, autoreleased LRStringEncoding object with the given string,
// as described below.
+ (id)stringEncodingWithString:(NSString *)string;

// Initialize a new LRStringEncoding object with the string.
//
...
FOUNDATION_EXPORT NSString *const LRStringEncodingErrorDomain;
FOUNDATION_EXPORT NSString *const LRStringEncodingBadCharacterIndexKey;  // NSNumber

typedef NS_ENUM(NSInteger, LRStringEncodingError) {
  // Unable to convert a buffer to NSASCIIStringEncoding.
  LRStringEncodingErrorUnableToConverToAscii = 1024,
  // Unable to convert a buffer to NSUTF8StringEncoding.
  LRStringEncodingErrorUnableToConverToUTF8,
  // Encountered a bad character.
  // LRStringEncodingBadCharacterIndexKey will have the index of the character.
  LRStringEncodingErrorUnknownCharacter,
  // The data had a padding character in the middle of the data. Padding characters
  // can only be at the end.
  LRStringEncodingErrorExpectedPadding,
  // There is unexpected data at the end of the data that could not be decoded.
  LRStringEncodingErrorIncompleteTrailingData,
};
